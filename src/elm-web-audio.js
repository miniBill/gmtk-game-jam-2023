...
class VirtualAudioGraph {
    ...
    for (const newNode of Object.values(newNodes)) {
      ...
      if (!oldNode) {
        ...
        newNode.connections.forEach((connection) => {
          patches.created.push({
            type: "connection",
            key: newNode.key,
            data: connection.key.split("."),
          });
        });
        ...
        for (
          let j = 0;
          j < Math.max(oldNode.properties.length, newNode.properties.length);
          j++
        ) {
        ...
        for (
          let j = 0;
          j < Math.max(oldNode.connections.length, newNode.connections.length);
          j++
        ) {
          ...
          if (!oldConnection) {
            patches.created.push({
              type: "connection",
              key: oldNode.key,
              data: newConnection.key.split("."),
            });
          } else if (!newConnection) {
            patches.removed.push({
              type: "connection",
              key: oldNode.key,
              data: oldConnection.key.split("."),
            });
  ...
  _createNode(key, { type, properties }) {
    ...
    switch (type) {
      ...
      case "ConvolverNode":
        ...
        break;
      case "DelayNode":
        const maxDelayTime = properties.find(
          ({ label }) => label === "maxDelayTime"
        );
        $node = this.$context.createDelay(
          (maxDelayTime && maxDelayTime.value) || 1
        );
        break;
      ...
      case "GainNode":
        ...
        break;
      case "IIRFilterNode":
        const feedforward = properties.find(
          ({ label }) => label === "feedforward"
        );
      ...
      case "MediaElementAudioSourceNode":
        const mediaElement = properties.find(
          ({ label }) => label === "mediaElement"
        );
      ...
      default:
        console.warn(
          `Invalide node type of: ${type}. Defaulting to GainNode to avoid crashing the AudioContext.`
        );
        $node = this.$context.createGain();
    }
  ...
  _setProperty(key, { type, label, value }) {
    ...
    switch (type) {
      case "NodeProperty":
        ...
        break;
      case "AudioParam":
        $node[label].linearRampToValueAtTime(
          value,
  ...
  _connect(a, [b, param = null]) {
    if (b)
      this.$nodes[a].connect(param ? this.$nodes[b][param] : this.$nodes[b]);
  }

  //
  _disconnect(a, [b, param = null]) {
    if (b)
      this.$nodes[a].disconnect(param ? this.$nodes[b][param] : this.$nodes[b]);
  }
}
